                ...
                public void run() {
                    try {
                        ...
                        Socket socket = poolExecutor.submit(() -> {
                            Socket socket2 = null;
                            try {
                                socket2 = fileMsgService.sendOfflineFileMsg(result);
                                UserMemory.users = JSON.parseArray(result.getObject().toString(), User.class);
                            } catch (Exception e) {
                                log.error(e.toString());
                            }
                            return socket2;
                        ...
                        Result result2 = poolExecutor.submit(new Callable<Result>() {
                            ...
                            public Result call() throws Exception {
                                ...
                                try (
                                    ...
                                    result2.setCode(Code.SEND_COMPLETED_OFFLINE_FILE_MSG);
                                    result2.setObject(fileMsg);

                                    socketPrintStream.println(result2);
